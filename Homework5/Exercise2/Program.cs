...
namespace Exercise2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Person with name Jerry
            Person JerryPerson = FansArray.Where(person => person.FirstName == "Jerry").FirstOrDefault();
            //All songs which start with the letter B
            List<Song> songWithB=Songs.Where(songTitle=>songTitle.Title.StartsWith("B")).ToList();
            JerryPerson.FavoriteSongs = songWithB;

            //Person with name Maria
            Person MariaPerson = FansArray.Where(person => person.FirstName == "Maria").FirstOrDefault();
            //Songs with length longer than 6 min
            List<Song> Songs6 = Songs.Where(song => (Convert.ToDouble(song.Length)/60 )>6.0).ToList();
            MariaPerson.FavoriteSongs = Songs6;

            //Person with name Jane
            Person JanePerson = FansArray.Where(person => person.FirstName == "Jane").First();
            //Songs of Rock genre
            List<Song> RockSongs = Songs.Where(song => song.Genre == Genre.Rock).ToList();
            JanePerson.FavoriteSongs = RockSongs;

            //Person with name Stefan
            Person StefanPerson = FansArray.Where(person => person.FirstName == "Stefan").First();
            List<Song> HipHop3 = Songs.Where(song => Convert.ToDouble(song.Length / 60) < 3.0).
                                       Where(song=>song.Genre==Genre.Hip_Hop).ToList();
            StefanPerson.FavoriteSongs = HipHop3;
            //StefanPerson.GetFavSongs();

            //All persons with 4 or more fav songs
            List<Person> FavSongs4 = FansArray.Where(songs => songs.FavoriteSongs.Count >= 4).ToList();
            //foreach (Person person in FavSongs4)
            //{
